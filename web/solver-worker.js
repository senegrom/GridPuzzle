...
self.onmessage=async({data:{id,puzzle}})=>{
  ...
  try{
    if(!runtime){
      ...
      runtime=await loadPyodide({indexURL:new URL('./vendor/pyodide/',self.location.href).href,stdout:()=>{},stderr:()=>{}});
      status('Loading the complete GridPuzzle solver…');
      const response=await fetch('./solver.zip');
      if(!response.ok)throw Error(`Solver download failed (${response.status}). Go online and retry.`);
    ...
    const result=JSON.parse(runtime.runPython('solve_json(_browser_payload)'));
    runtime.globals.delete('_browser_payload');
    self.postMessage({id,type:'result',result});
  }catch(error){runtime=null;self.postMessage({id,type:'result',result:{status:'error',message:error.message||String(error)}});}
};
