...
- **Play:** `play-actions.test.js` runs the production action and worker handlers with controlled browser I/O: cached-result races, superseded requests, construction and postMessage failures, worker errors and deadlines. `editor-followup.test.js` covers Play hiding full-solution photo overlays.
...
- **Workers and robustness:** `worker-lifecycle.test.js` covers OCR and scan cancellation and worker cleanup; `review-hardening.test.js` and `final-hardening.test.js` pin the individual fixes from the code reviews (for example a captured still surviving page hide, Escape restoring focus, the saved-picture download reusing its object URL, a running task disabling Check and Hint, and the solver worker keeping its interpreter across Python exceptions).
