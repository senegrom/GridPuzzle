...
const symbol = (text, x0, x1, confidence = 98) => ({
  text,
  confidence,
  bbox: { x0, x1, y0: 20, y1: 90 },
});

test("A whole atlas row recognized as one word still maps each digit to its cell", () => {
  const word = {
    ...symbol("538", 25, 310, 72),
    symbols: [
      symbol("5", 25, 75),
      symbol("3", 137, 188),
      symbol("8", 249, 310),
  ...
  assert.deepEqual(
    mapAtlas(data([word]), 3, 3, 112).map((x) => x.text),
    ["5", "3", "8"],
  );
  assert.deepEqual(
    mapAtlas(data([word]), 3, 3, 112).map((x) => x.confidence),
    [98, 98, 98],
  );
});
test("Multidigit and operator clues stay together INSIDE their own atlas tile", () => {
  ...
  assert.deepEqual(
    mapAtlas(data([word]), 2, 2, 112).map((x) => x.text),
    ["12+", "7"],
  );
});
test("Unsegmented cross-tile words and crossing symbols require review", () => {
  ...
  assert.ok(
    result.every((r) => r.review && r.confidence === 0 && r.text === ""),
  );
});
test("Missing output remains unread, not guessed", () => {
  assert.deepEqual(mapAtlas({}, 1, 1, 112), [
    { text: "", confidence: 0, review: false },
  ]);
});
test("A low-confidence one-digit word remains uncertain despite a confident symbol", () => {
  const word = {
    ...symbol("4", 20, 80, 40),
    symbols: [symbol("4", 20, 80, 99)],
  };
...
test("Compact atlas layout stays within the mobile raster budget", () => {
  ...
  for (const n of [81, 256, 625, 1200, 1800]) {
    const a = atlasLayout(n);
    assert.ok(a.columns * a.rows * a.tile * a.tile <= 8_000_000);
    assert.ok(a.tile >= 64);
  }
