...
async function result(page){
  await page.waitForFunction(async()=>{const app=await import('./app.js');const s=app.getState();return !s.busy&&s.result!==null;},{},{timeout:150000});
  return page.evaluate(async()=>{const app=await import('./app.js');return app.getState().result;});
}
...
(async()=>{
  ...
  for(const [name,engine] of Object.entries({chromium,webkit})){
    ...
    try{
      ...
      const edited=await page.evaluate(async()=> (await import('./app.js')).getState());assert.equal(edited.puzzle.cells[0],9);assert.equal(edited.result,null);await page.click('#undo');assert.equal((await page.evaluate(async()=> (await import('./app.js')).getState())).puzzle.cells[0],5);report.checks.push('cell editing and undo');
      ...
      assert.equal((await page.evaluate(async()=> (await import('./app.js')).getState())).busy,false);assert.equal((await page.evaluate(async()=> (await import('./app.js')).getState())).result,null);await load(page,'sudoku');await page.click('#solve');assert.equal((await result(page)).status,'unique');report.checks.push('worker cancellation and clean restart');
      ...
      await page.waitForFunction(async()=>{const s=(await import('./app.js')).getState();return !s.busy&&s.puzzle.cells.some(Number.isInteger);},{},{timeout:150000});
      const scan=await page.evaluate(async()=>{const app=await import('./app.js'),model=await import('./model.js'),s=app.getState(),reference=model.demo().cells;return {type:s.puzzle.type,recognized:s.puzzle.cells.filter(Number.isInteger).length,correct:s.puzzle.cells.filter((v,i)=>v!==null&&v===reference[i]).length,unsafe:s.puzzle.cells.flatMap((v,i)=>v!==null&&v!==reference[i]&&!s.uncertain.includes(i)?[i]:[]),uncertain:s.uncertain};});
      ...
      // Solve a confirmed transcription to exercise photograph mapping independently
      // of OCR recall. This does not silently correct production recognition.
      if((await page.evaluate(async()=> (await import('./app.js')).getState())).result===null){await page.click('#solve');if(await page.locator('#confirm-dialog').isVisible())await page.click('#confirm-solve');await result(page);}
      ...
      await page.locator('#prepare-offline').evaluate(el=>{el.closest('details').open=true;});await page.click('#prepare-offline');await page.waitForFunction(()=>document.querySelector('#offline-state').textContent.startsWith('Offline assets are ready'),{},{timeout:300000});
      await context.setOffline(true);await page.reload();await page.waitForSelector('body[data-ready="true"]');await load(page,'sudoku');await page.click('#solve');assert.equal((await result(page)).status,'unique');report.checks.push('offline reload and Python solve');
