...
def main():
    ...
    ap.add_argument("--out", type=Path, default=Path("artifacts/photos"))
    a = ap.parse_args()
    root = a.source / "Examples/BrowserScanner/Newspaper"
    ...
    for f in json.loads((root / "ground-truth.json").read_text())["fixtures"]:
        ...
        for name in ["original", "small", "faded", "mild-fade", "blur"]:
            ...
            if name == "small":
                im = im.resize(
                    (round(im.width / 2), round(im.height / 2)),
            ...
            if name in ["faded", "mild-fade"]:
                contrast = 0.35 if name == "faded" else 0.65
                pixels[:, :, :3] = np.floor(
                    255 - (255 - pixels[:, :, :3].astype(float)) * contrast + 0.5
                )
            path = f"{f['name']}-{name}.rgba"
            (a.out / path).write_bytes(pixels.tobytes())
            cases.append(
                {
                    "name": f["name"],
                    "variation": name,
