...
def insert_after_in_section(
    ...
    target_index = text.index(target, section)
    if text.find(target, target_index + 1) != -1 and label == "layered path method":
        # Multiple apply methods are expected in topology.py; section scoping
        # deliberately picks the first one in ConsecutiveAdjacencyRule.
        pass
    path.write_text(
        text[:target_index] + addition + text[target_index:],
        encoding="utf-8",
    )
...
NEW_ACTIONS = '''    def _generic_power_actions(self) -> Iterator[str]:
...
INTERVAL_METHOD = '''    def _prune_layered_intervals(
        self,
...
CYCLIC_BLOCKS_METHOD = '''    def _cyclic_blocks(self, possible: set[int]) -> tuple[frozenset[int], ...]:
...
replace_once(
    ...
    CYCLIC_BLOCKS_METHOD + "    def _remove_selected_value(\n",
    "cyclic block method",
)
replace_once(
    TOPOLOGY,
