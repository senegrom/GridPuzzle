...
def xy_wing(grid: Grid) -> None:
    ...
    links1 = {c: {i: {c2 for c2, cd2 in cd2_cells if cd2 == {min(cd), i} and c2 in wl[c]} for i in
                  range(1, grid.max_elem + 1)} for c, cd in cd2_cells}
    links2 = {c: {i: {c2 for c2, cd2 in cd2_cells if cd2 == {max(cd), i} and c2 in wl[c]} for i in
                  range(1, grid.max_elem + 1)} for c, cd in cd2_cells}
...
def xyz_wing(grid: Grid) -> None:
    ...
    for c, cd in cd3_cells:
        links = {
            frozenset([x1, x2]): {c2 for c2, cd2 in cd2_cells if cd2 == {x1, x2} and c2 in wl[c]}
