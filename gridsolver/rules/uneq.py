...
class SingleRelationRule(Rule, ABC):
    ...
    def __init__(
        ...
        # set, so equivalent input orders hash and compare identically. Complete
        # neighbourhood descriptions may cross grid edges; only those valid,
        # same-form related cells are clipped. The origin stays strict.
        super().__init__(
            gsz,
            [origin_cell, *related],
            None,
            _clip_outside_after_first=True,
        )
        if len(self.cells) < 2:
            raise ValueError(
